...
def get_bufset(paths, bufs, with_right_unit=False):
    ...
    for buf in bufs:
        out[buf] = np.array(
            [
                read_img(path, buf, with_right_unit=with_right_unit)
                for path in paths
            ]
        )
    return out
...
def get_src_data(paths, buffers, gauss=None, sec=False, with_right_unit=False):
    ...
    buffer_set_raw = get_bufset(
        paths, bufs=buffers, with_right_unit=with_right_unit
    )
...
def main3():
    ...
    # standardize data
    # reg_buffer_set = standardize_data(reg_buffer_set)
