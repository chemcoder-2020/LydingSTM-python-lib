#!/Users/Ordinary/anaconda/bin/python
...
# Conversion: AD -2.75V 2.75V 16-bit
# def timeit(method):
#     @wraps(method)
#     def timed(*args, **kw):
#         ts = time()
...
class ACitsBlock:
    def __init__(self,block_data_type,block_mode,data,block_number,cits_spec_raw_deglitch,cits_spec_raw_glitch_threshold,cits_spec_raw_smooth,\
                 cits_spec_raw_smooth_order,cits_spec_raw_smooth_n_fit):
...
class ASpecBlock:
    def __init__(self,max_spec_per_coord,
        ...
        self.coords = []
        self.stsdata = []
        self.vrange = np.linspace(self.spec_vstrt,self.spec_vfnsh,num=self.spec_pt_per_spec)
...
class STMfile:
    def __init__(self,file_path):
        ...
        self.xst,self.xfin,self.xinc,self.x_ofset,self.yst,self.yfin,self.yinc,self.y_ofset,self.scnxin,\
            self.scnyin,self.theta,self.scan_del = self.parseChunk(b"SCAN    ","<"+"f"*12,skip_steps=2)
        self.scan_ad_check,self.scan_up_down,self.scanning_up,self.nsampl,self.nscans,\
            self.xnum,self.ynum = self.parseChunk(b"SCAN    ","<"+"h"*7,skip_steps=50)
        self.xcal,self.ycal,self.zcal,self.xver,self.yver,self.zver = \
            self.parseChunk(b"CAL     ","<"+"f"*6)
        self.cur_gain, self.hv_gain, self.amplif = self.parseChunk(b"GAIN_1  ","<"+"f"*3)
        self.top_ad_ver,self.top_ad_max_gain,self.cur_ad_ver,self.cur_ad_gain,self.err_ad_ver,\
            self.err_ad_gain,self.lock_ad_ver,self.lock_ad_gain = \
            self.parseChunk(b"ATOD    ","<"+"f"*8)
        ...
        try:
            self.prop_gain,self.intg_gain,self.der_gain,self.atod1_gain,self.atod2_gain\
                = self.parseChunk(b"ELEC1   ","<"+"f"*5)
        except Exception:
            self.prop_gain, self.intg_gain, self.der_gain, self.atod1_gain, self.atod2_gain = \
                (None,None,None,None,None)
        try:
            self.atod1_chanl, self.atod2_chanl = self.parseChunk(b"ELEC1   ","<"+"h"*2,skip_steps=20)
        except Exception:
            self.atod1_chanl, self.atod2_chanl = (None, None)
        self.valid_spec,self.bias_to_probe,self.cits_on = self.parseChunk(b"FLAG    ","<"+"h"*3)
        ...
        # specify some titles
        _titles = {0: "Current Buffer: ", 1: "Topographic Buffer: ",
                  2: "Dig. Topographic Buffer: ", 3: "Error Image: ",
                  4: "Lock-In Image: ", 5: "d2I/dV2 Buffer: "}
        self.BufTitles = {}
        ...
        if self.valid_spec == 1 and self.valid_cits == 0:
            self.spec_num_blocks, self.spec_array_offset,self.spec_max_points_per_spec,\
            self.spec_max_spec_per_coord, self.spec_max_num_spec, self.spec_active_block=\
            self.parseChunk(b'SPEC_M1','<hfhhhh')
            if self.spec_num_blocks > 0:
                for i in range(self.spec_num_blocks):
                    _tmp = self.parseChunk(b'SPEC_M1','<h80s8hfh22fh4fh2f2hf2h3f6h6fh2f7h',skip_steps=14)
                    spec_block = ASpecBlock(2,*_tmp)
                    for j in range(spec_block.spec_num_spec):
                        spc = 1
                        if spec_block.spec_potentiometry and (spec_block.spec_mode==1 or spec_block.spec_mode==4):
                            spc = 2
                    ...
                    spec_block.coords = np.array(self.parseChunk(b'SPC_MDAT','<{a}h'.format(a=_parsed),skip_steps=_skipped))
                    spec_block.coords = spec_block.coords.reshape((2,spec_block.spec_num_spec))
                    spec_block.coords = np.stack((spec_block.coords[0,:],spec_block.coords[1,:]),axis=-1)
                    spec_block.coords[:,1] = self.ynum - spec_block.coords[:,1]
                    self.stsblocks[i] = spec_block

                for i in range(self.spec_num_blocks):
                    _skipped = self.stsblocks[i].spec_num_spec * 2 * self.spec_num_blocks * struct.calcsize('<h')
                    if self.stsblocks[i].max_spec_per_coord == 1:
                        _skipped1 = self.stsblocks[i].spec_pt_per_spec * self.stsblocks[i].spec_num_spec * i * struct.calcsize('<f')
                        self.stsblocks[i].stsdata = self.parseChunk(b'SPC_MDAT','<{a}f'.format(a=self.stsblocks[i].spec_pt_per_spec * self.stsblocks[i].spec_num_spec),
                                                             skip_steps=_skipped+_skipped1)
                        self.stsblocks[i].stsdata = np.reshape(self.stsblocks[i].stsdata, (self.stsblocks[i].spec_num_spec,1,self.stsblocks[i].spec_pt_per_spec) )
                        self.stsblocks[i].stsdata = self.stsblocks[i].stsdata * 2 * float(self.dsp_atod_max_v) / 2**16 / self.cur_gain / self.cur_ad_gain
                    elif self.stsblocks[i].max_spec_per_coord == 2:
                        _skipped1 = self.stsblocks[i].spec_pt_per_spec * self.stsblocks[i].spec_num_spec * 2*i * struct.calcsize('<f')
                        self.stsblocks[i].stsdata = self.parseChunk(b'SPC_MDAT',
                                                             '<{a}f'.format(a=2*self.stsblocks[i].spec_pt_per_spec),
                                                             skip_steps=_skipped + _skipped1)
                        self.stsblocks[i].stsdata = np.reshape(self.stsblocks[i].stsdata,(self.stsblocks[i].spec_num_spec, 2, self.stsblocks[i].spec_pt_per_spec))
                        self.stsblocks[i].stsdata = self.stsblocks[i].stsdata * 2 * float(self.dsp_atod_max_v) / 2 ** 16 / self.cur_gain / self.cur_ad_gain
        ...
        if self.valid_cits == 1:
            self.cits_spec_mode, self.cits_num_buff, self.cits_oversample_mult, self.cits_spec_vstrt, self.cits_spec_vfnsh, \
            self.cits_spec_pt_del, self.cits_spec_avg_num_spec, self.cits_spec_avg_del, self.cits_spec_lock_in_point_delay \
                = self.parseChunk(b'CITS', "<hhhfffhff")
            self.cits_bias = []
            _cits_bias_start_ind = self.find(b'CITS') + 8 + 2 * 3 + 4 * 3 + 2 + 4 * 2
            for i in range(self.cits_num_buff):
                self.cits_bias.append(
                    struct.unpack("<f", self.bin_data[_cits_bias_start_ind + 4 * i:_cits_bias_start_ind + 4 * i + 4])[
                        0])

            if self.find(b'CITS_1') != -1:
                self.cits_spec_dither_only, self.cits_topo_average, self.cits_topo_num_average, self.cits_spec_fit, self.cits_spec_fit_pts = \
                    self.parseChunk(b'CITS_1', "<hhhhh")
            if self.find(b'CITS_2') != -1:
                self.cits_num_blocks, self.cits_analysis_block, self.cits_display_block = self.parseChunk(b"CITS_2", "<hhh")
            if self.find(b'CITS_3') != -1:
                self.cits_spec_istrt, self.cits_spec_ifnsh, self.cits_spec_zstrt, self.cits_spec_zfnsh, self.cits_spec_zscan = \
                    self.parseChunk(b'CITS_2', "<fffff")
            if self.find(b'CITS_5') != -1:
                self.cits_log_temperature = self.parseChunk(b'CITS_5', "<h")[0]
                if self.cits_log_temperature:
                    self.cits_temperature_pts, self.cits_temperature_atod_channel, self.cits_temperature_log_interval, \
                    self.cits_temperature_log_sampl_dt, self.cits_temperature_conv_factor = self.parseChunk(b'CITS_5',
                                                                                                            "<hhfff",
                                                                                                            skip_steps=2)
            if self.find(b'CITS_6') != -1:
                self.cits_spec_dither_only, self.cits_topo_average, self.cits_topo_num_average = self.parseChunk(b'CITS_6',
                                                                                                                 "<hhh")
            _cits_block_mode = []
            ...
            for i in range(self.cits_num_blocks):
                _skipped = 2*3+2*i
                _cits_block_mode.append(self.parseChunk(b'CITS_2','<h',skip_steps=_skipped)[0])
            ...
            for j in range(self.cits_num_blocks):
                _tmp = self.parseChunk(b'CITS_6', "<5h", skip_steps=6 + j * 5 * struct.calcsize('<h'))  # skip 3 integers after cits_6 flag
                if _cits_block_data_type[j] == 0:
                    for i in range(self.cits_num_buff):
                        _skipped = self.dimensions[0] * self.dimensions[1] * (struct.calcsize('<h') * self.numbufs +\
                                                                              struct.calcsize('<h') * i)
                        _arr = self.parseChunk(b'IMG_BUF', "<{j}h".format(j=self.dimensions[0] * self.dimensions[1]),
                                               skip_steps=_skipped)
                        _arr = np.reshape(_arr, (self.ynum, self.xnum))
                        _arr = np.flip(_arr, 0)
                        _intdata.append(_arr)
                    _intdata = np.array(_intdata)
                    _intdata = _intdata * self.dsp_atod_max_v * 2 / 2**16 / self.cur_gain / self.cur_ad_gain
                    _ABlock = ACitsBlock(_cits_block_data_type[j], _cits_block_mode[j], _intdata, j, *_tmp)
                    ...
                    for i in range(self.cits_num_buff):
                        _skipped = self.dimensions[0] * self.dimensions[1] * (struct.calcsize('<h') * self.numbufs +
                                                                              struct.calcsize('<f') * i)
                        _arr = self.parseChunk(b'IMG_BUF', "<{j}f".format(j=self.dimensions[0] * self.dimensions[1]),
                                               skip_steps=_skipped)
                        _arr = np.reshape(_arr, (self.ynum, self.xnum))
                        _arr = np.flip(_arr, 0)
                        _realdata.append(_arr)
                    _realdata = np.array(_realdata)
                    _realdata = _realdata * self.dsp_atod_max_v * 2 / 2 ** 16 / self.cur_gain / self.cur_ad_gain
                    _ABlock = ACitsBlock(_cits_block_data_type[j], _cits_block_mode[j], _realdata, j, *_tmp)
                    self.cits_blocks[j] = _ABlock
    ...
    def findDimension(self):
        ...
        if i == -1: raise Exception("Missing data")
        return struct.unpack("<HH",self.bin_data[i-4:i]) # this will be a tuple of 2 numbers
    ...
    def parseChunk(self, header, format,skip_steps=0):
        ...
        i = self.find(header)
        if i == -1: raise Exception("missing data")
        return struct.unpack(format, self.bin_data[i + 8 + skip_steps:i + 8 + skip_steps + struct.calcsize(
            format)])  # There are 8 spaces from the header to the actual data
    ...
    def buildGwyDataLine(self, stsblocknum, sts_spec_num):
        ...
        if stsblocknum >= self.spec_num_blocks:
            raise Exception("addGwyDataline went out of range of number of linenum (stsblocks)")
        elif sts_spec_num >= self.stsblocks[stsblocknum].spec_num_spec:
            raise Exception("addGwyDataline went out of range of number of linenum (spec_num_spec)")
        # res
        chanByte = self.convStringToByte("res\0")
        chanByte += self.convCharToByte('i')
        if self.stsblocks[stsblocknum].max_spec_per_coord == 2:
            chanByte += self.convIntToByte(self.stsblocks[stsblocknum].spec_pt_per_spec * 2)
        else:
        ...
        chanByte += self.convDoubleToByte(np.abs(self.stsblocks[stsblocknum].spec_vstrt -
                                                 self.stsblocks[stsblocknum].spec_vfnsh))

        # off
        chanByte += self.convStringToByte("off\0")
        chanByte += self.convCharToByte('d')
        chanByte += self.convDoubleToByte(np.min((self.stsblocks[stsblocknum].spec_vstrt,
                                                 self.stsblocks[stsblocknum].spec_vfnsh)))
        ...
        if self.stsblocks[stsblocknum].max_spec_per_coord == 2:
            ...
            chanByte += self.convIntToByte(self.stsblocks[stsblocknum].spec_pt_per_spec,
                                           format="<I")
        if self.stsblocks[stsblocknum].spec_vstrt > self.stsblocks[stsblocknum].spec_vfnsh:
            chanByte += (np.flip(np.array(self.stsblocks[stsblocknum].stsdata[sts_spec_num,:,:]).flatten())).tobytes()
        else:
            chanByte += (np.array(self.stsblocks[stsblocknum].stsdata[sts_spec_num,:,:]).flatten()).tobytes()
        return chanByte, len(chanByte)
    ...
    def buildGwySpectra(self):
        ...
        _no_coords = self.spec_num_blocks * 2 * self.stsblocks[0].spec_num_spec
        coordByte += self.convIntToByte(_no_coords, format='<I')
        _coords = np.array([self.stsblocks[i].coords for i in range(self.spec_num_blocks)]).flatten()
        coordByte += ((self.xfin-self.xst)/self.xnum * 1E-10 * _coords).tobytes()

        # data
        coordByte += self.convStringToByte("data\0",format='ascii')
        coordByte += self.convCharToByte('O')
        coordByte += self.convIntToByte(self.spec_num_blocks * self.stsblocks[0].spec_num_spec, format='<I')
        _tmpbyte = []
        _lenbyte = []
        for blocknum in range(self.spec_num_blocks):
            for num_spec in range(self.stsblocks[0].spec_num_spec):
                _tmpvar = self.addGwyDataLine(blocknum,num_spec)
                _tmpbyte.append(_tmpvar[0])
                _lenbyte.append(_tmpvar[1])

        for i in range(self.spec_num_blocks * self.stsblocks[0].spec_num_spec):
            coordByte += _tmpbyte[i]
        return coordByte, len(coordByte)

    def addGwySpectra(self):
        spectraField = self.convStringToByte("/sps/0\0", format='ascii')
        spectraField += self.convCharToByte("o")
        spectraField += self.convStringToByte('GwySpectra\0', format='ascii')
        spectra, length_of_it_spectra = self.buildGwySpectra()
    ...
    def brickDataByteObject(self,bricknum):
        if bricknum >= self.cits_num_blocks:
            raise Exception("brickDataByteObject went out of range of number of CITS blocks")
        # xres
        ...
        brickByte += self.convCharToByte('D')
        brickByte += self.convIntToByte(self.xnum * self.ynum * self.cits_num_buff, format="<I")
    ...
    def addGwyBrick(self,bricknum):
       ...
       brickField += self.convStringToByte("/brick/%i/preview\0" % bricknum, format='ascii')
       brickField += self.convCharToByte("o")
       brickField += self.convStringToByte('GwyDataField\0', format='ascii')
       chanByteObject, length_of_it_field = self.channelDataByteObject(0) # show topographic buffer in preview
    ...
    def addBrickTitle(self,bricknum):
        ...
        BrickTitle = self.convStringToByte('/brick/%i/title\0' % bricknum,format='ascii')
    ...
    def channelDataByteObject(self,bufnum):
        ...
        if bufnum >= self.numbufs:
            raise Exception("channelDataByteObject went out of range of number of buffers")
        # xres
        ...
        chanByte += self.convStringToByte("data\0")
        chanByte += self.convCharToByte('D')
        chanByte += self.convIntToByte(self.xnum*self.ynum,format="<I") # times 8 or not? Maybe just number of items in array
        ...
        if self.BufDataTypes[bufnum] == 0:
            if self.buf_scan_modes[bufnum] == 0:
                mulfac = (self.dsp_atod_max_v / self.dtoa_max_value) / self.cur_ad_gain ** 2
        ...
        chanByte += (self.get_buffers([bufnum+1])[bufnum+1] * mulfac * 1E-10 + addfac).tobytes()
        return chanByte, len(chanByte)
    ...
    def addGwySelection(self,bufnum):
        ...
        GwySelection = self.convStringToByte("/%i/select/pointer\0" % bufnum,format='ascii')
    ...
    def addGwyPalette(self,bufnum):
        ...
        GwyPalette = self.convStringToByte('/%i/base/palette\0' % bufnum, format='ascii')
    ...
    def toGwy(self):
        ...
        datasize = sum([GwyVises[i][1] for i in range(len(GwyVises))]) + \
        ...
        if self.valid_cits == 1:
            ...
            datasize += sum([GwyBricks[i][1] for i in range(len(GwyBricks))]) + \
                            sum([GwyBrickTitles[i][1] for i in range(len(GwyBrickTitles))])

        elif self.valid_spec == 1 and self.valid_cits == 0:
            GwySpectra.append(self.addGwySpectra())
            datasize += sum([GwySpectra[i][1] for i in range(len(GwySpectra))])

        with open(self.fp + '.gwy','wb') as f:
            f.write(GwyHeader)
            f.write(self.convIntToByte(datasize,format='<I')) # size of the entire data file
            f.write(GwyFn)
    ...
    def get_all_orig_buffers(self):
        ...
        if i == -1: raise Exception("Missing data")
        buffer_starts = [i + 8 + 2 * b * self.dimensions[0] * self.dimensions[1] for b in range(self.numbufs)]
        buffers = {}
            ...
            if self.BufDataTypes[elem] == 0:
                for x in range(self.xnum):
                    for y in range(self.ynum):
                        img_arr[y,x] = struct.unpack("<h",self.bin_data[buffer_starts[idx]:buffer_starts[idx]+2])[0]
    ...
    def get_buffers(self,bufferList):
        ...
        """
        for i in bufferList:
            if i > self.numbufs: raise Exception("One or more buffer number(s) are not within the total number of buffers")
        i = self.bin_data.find(b"IMG_BUF ")
        if i == -1: raise Exception("Missing data")
        buffer_starts = [i + 8 + 2*(b-1)*self.dimensions[0]*self.dimensions[1] for b in bufferList]
        ...
        for idx,elem in enumerate(bufferList):
            if self.BufDataTypes[elem-1] == 0:
                ...
                image_array = struct.unpack("<"+"h"*points,self.bin_data[buffer_starts[idx]:buffer_starts[idx]+points*2])
    ...
    def get_height_buffers(self,bufferList):
        ...
        height_buffers = {}
        for key in buffers:
            height_buffers[key] = buffers[key] * 6 / 2**16 * self.zver / 10 * self.zcal * self.hv_gain * 3.35 # in Angstrom
...
def parseChunk(bindat, header, format,skip_steps=0):
    ...
    if i == -1: raise Exception("missing data")
    return struct.unpack(format, bindat[i + 8 + skip_steps:i + 8 + skip_steps + struct.calcsize(format)])  # There are 8 spaces from the header to the actual data
