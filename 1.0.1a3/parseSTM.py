...
class STMfile:
    ...
    def buildGwyDataLine(self, stsblocknum, sts_spec_num):
        ...
        if self.stsblocks[stsblocknum].spec_vstrt > self.stsblocks[stsblocknum].spec_vfnsh:
            chanByte += deglitch_and_shift_1d_array(np.flip(
                np.array(self.stsblocks[stsblocknum].stsdata[sts_spec_num,:,:]).flatten()),
                self.stsblocks[stsblocknum].spec_vstrt,self.stsblocks[stsblocknum].spec_vfnsh,
                self.stsblocks[stsblocknum].spec_pt_per_spec).tobytes()
        else:
            chanByte += deglitch_and_shift_1d_array(
                np.array(self.stsblocks[stsblocknum].stsdata[sts_spec_num,:,:]).flatten(),
                self.stsblocks[stsblocknum].spec_vstrt, self.stsblocks[stsblocknum].spec_vfnsh,
                self.stsblocks[stsblocknum].spec_pt_per_spec).tobytes()
        return chanByte, len(chanByte)
    ...
    def buildGwySpectra(self,stsblocknum):
        ...
        coordByte += self.convCharToByte('O')
        coordByte += self.convIntToByte(self.stsblocks[stsblocknum].spec_num_spec, format='<I')
        _tmpbyte = []
        _lenbyte = []
    ...
    def addGwySpectra(self,stsblocknum):
        spectraField = self.convStringToByte("/sps/{i}\0".format(i=stsblocknum), format='ascii')
...
def deglitch_and_shift_1d_array(arr, vstart, vfnsh, spec_pt_per_spec):
    # zero volt index
    shift_ind = int(np.abs( 0 - vstart ) / np.abs(vfnsh-vstart)  * spec_pt_per_spec)
    print(shift_ind)

    # deglitch
    arr[shift_ind-5:shift_ind+5] = medfilt(arr[shift_ind-5:shift_ind+5], kernel_size=11)
    print(arr[shift_ind])
    arr -= arr[shift_ind]
    return arr
